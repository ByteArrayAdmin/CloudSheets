...
const ForgotPassword = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    setLoader(true);
    forgetPassword_sendEmail(email)
      .then((response: any) => {
  ...
  const onUpdatePassword = async (data: any) => {
    ...
    setLoader(true);
    forgetPassword_submit(email, code, newPassword)
      .then((response: any) => {
                    ...
                    <View>
                      ...
                      <View>
                        <InputField
                          ...
                          control={control2}
                          placeholder={
                            Forgotlabel.Forgotpassword.ENTER_NEW_PASSWORD
                          }
                          Image={Lock}
                          rules={{
                            required:
                              Forgotlabel.Forgotpassword.NEW_PASSWORD_REQUIRED,
                            minLength: {
                            ...
                            pattern: {
                              value:
                                /^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[A-Z]).+$/, // Regular expression pattern for at least one number, one special character, and one uppercase letter
                              message:
                                "Password must contain at least one number, one special character, and one uppercase letter.",
                            },
                          }}
